...
program
  .command("deploy")
  .description(
    "Upload local source, create a deployment, and follow retained build events",
  )
  .argument("[directory]", "source directory", ".")
  .requiredOption("--project <id>", "project resource ID")
  ...
  .action(
    async (options: {
      ...
      const terminal = await followOperation(
        ...
        Boolean(options.json),
        nonnegativeInteger(options.after, "event sequence"),
      );
      assertSuccessful(terminal);
    },
  );
...
program
  ...
  .action(
    async (options: {
      ...
      const result = await clientFor(options).cancelDeployment(
        options.deployment,
        `cli-cancel-${randomUUID()}`,
        options.reason,
      );
      process.stdout.write(`${JSON.stringify(result)}\n`);
    },
...
async function uploadSource(
  client: LrailClient,
  projectId: string,
  archive: Awaited<ReturnType<typeof createSourceArchive>>,
...
async function followOperation(
  ...
  for (;;) {
    const batch = await client.getOperationEvents(
      operationId,
      generation,
      sequence,
    );
    for (const event of batch.data) printEvent(event, json);
    ...
    if (isTerminal(batch.operation)) return batch.operation;
    await delay(750);
  }
}
...
function assertSuccessful(operation: OperationResource): void {
...
function isTerminal(operation: OperationResource): boolean {
  return ["succeeded", "failed", "canceled", "waiting"].includes(
    operation.state,
  );
}
