...
- `build-control`: signed assignment verification, Starlark/Build IR/LLB, disposable Kata BuildKit workers, policy egress, offline SBOM/scan policy, immutable Harbor publication, and residue quarantine
- `build-registry-broker`: private Harbor tenancy plus short-lived repository token issuance; the only build-cell unit with Harbor administrator authority
- `build-evidence-signer`: mTLS-only DSSE/Cosign signing through a non-exportable OpenBao Transit key
