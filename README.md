...
The separate Build IR policy compiler pins official BuildKit LLB, performs no solve or ambient I/O, and fails closed before customer execution. It binds organization/project scope, Build IR digest, versioned policy, canonical non-secret arguments, digest-pinned base-material resolution/SBOM/provenance/signature evidence, network gateway/host intent, tenant/trust/policy-scoped cache IDs, logical secret mounts, and output config into one canonical definition lock. Native source, image, exec, secret/cache mount, and file-copy vertices produce deterministic protobuf definitions and golden graph digests. Secret values, worker identity, timestamps, queue data, and provider request IDs never enter the request, LLB, cache key, output config, or definition digest.
