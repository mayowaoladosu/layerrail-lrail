...
- the final public, digest-pinned controller, worker, egress-proxy, and residue-agent images

The example overlay contains only non-authentic placeholders. Replace its public key, CA, endpoint, object credential, and `lrail-build-images` Docker config through the production secret/configuration workflow. The package token needs read-only access to these four private component packages and no repository or write scope. Never commit real credentials or private key material.
